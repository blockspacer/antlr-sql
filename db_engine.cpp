...
Table* db_engine::find_table_by_name(string fl) {
	...
	cerr << "No tables with this name currently open\n";
	return NULL;
}
...
Table db_engine::natural_join(Table *t1, Table *t2) {
	...
	for (int i=0; i<similar_attributes.size(); ++i) {				//remove duplicate attributes
		for (int j=0; j<new_attributes.size(); ++j) {
			if (new_attributes[j] == similar_attributes[i]) {
				new_attributes.erase(new_attributes.begin() + j);
				new_types.erase(new_types.begin() + j);
				j = new_attributes.size();
			}
		}
	}
	ret.attributes = new_attributes;
